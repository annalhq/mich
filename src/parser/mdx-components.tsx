...
const components: MDXComponents = {
  PreviewExample: () => {
    return (
      <div className="min- border-yellow-6 bg-yellow-3 text-yellow-11 flex h-10 w-32 items-center justify-center rounded-lg border">
  ...
  blockquote: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => (
    <blockquote
      className={cn("border-gray-4 mt-6 border-l-2 pl-6 text-muted", className)}
      {...props}
    />
  ),
  ...
  ol: ({ className, ...props }: React.HTMLAttributes<HTMLOListElement>) => {
    if (
      ...
      return (
        <ol data-footnotes>
          <div className="text-small mb-2 mt-6 text-muted">Footnotes</div>
          {props.children}
        </ol>
      );
    }
  ...
  li: ({
    ...
    return <li className={cn("ml-2 mt-2 list-item", className)}>{children}</li>;
  },
};
